...
| 5 | `post-image` | Builds a ready-to-paste image prompt in one of two hand-drawn sketch styles (`sketch-on-white` / `sketch-on-black`), with the hook overlaid as text and a tactile, content-specific metaphor. Sets the draft's `concept_path`. | `concepts/<date>-<slug>/prompt.md` |
